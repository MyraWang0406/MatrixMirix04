...
【核心】CreativeSet / EvaluationSet 本质是 CreativeCard 的组合。
- 每张卡 = 一组结构变量（hook / sell_point / CTA / 动机桶 等）
- 不是一个 mp4；视频是渲染结果，不参与结构胜率统计
"""
...
class StructureEvaluationSet:
    """
    结构评测集（离线）：一组 CreativeCard / StrategyCard。
    只关心结构字段，用于统计结构胜率。非视频集合。
    """
