...
def main():
    ...
    with hc1:
        st.markdown('<span class="ds-header-title" style="color:#fff;font-weight:700;font-size:1.6rem;letter-spacing:0.02em;">决策看板 (Decision Board)</span>', unsafe_allow_html=True)
    with hc2:
        if st.button("决策看板", type="primary" if view == "决策看板" else "secondary", key=f"{K}tab_board"):
    ...
    with hc5:
        if st.button("复盘检索", type="primary" if view == "复盘检索" else "secondary", key=f"{K}tab_review"):
            ...
            st.rerun()
    with hc6:
        st.markdown('<span style="color:rgba(255,255,255,0.95);font-size:0.85rem;">行业</span>', unsafe_allow_html=True)
    with hc7:
        if st.button("休闲游戏", type="primary" if vert_idx == "休闲游戏" else "secondary", key=f"{K}vert_game"):
            st.session_state[f"{K}vertical"] = "休闲游戏"
            st.session_state[f"{K}use_generated"] = False
            st.session_state[f"{K}generated_variants"] = None
            st.rerun()
    with hc8:
        if st.button("电商", type="primary" if vert_idx == "电商" else "secondary", key=f"{K}vert_ec"):
    ...
    with st.expander("筛选与生成", expanded=True):
