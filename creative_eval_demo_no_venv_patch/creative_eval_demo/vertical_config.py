...
def get_why_you_phrase_list(vertical: str = "casual_game") -> list[str]:
    """
    给 variant_suggestions.py 用的 why_you 短语列表。
    兼容你现在已有的 get_corpus / get_why_you_examples 两套数据结构。
    """
    # 1) 优先从 corpus 里取（如果你 corpus 里有维护 why_you_* 相关字段）
    try:
        corpus = get_corpus(vertical) or {}
        for k in ["why_you_phrase_list", "why_you_phrases", "why_you_bucket", "why_you"]:
            v = corpus.get(k)
            if isinstance(v, list) and v:
                # list[str] 直接返回
                if all(isinstance(x, str) for x in v):
                    return v
                # list[dict] 提取 phrase/text/name
                out = []
                for x in v:
                    if isinstance(x, dict):
                        out.append(x.get("phrase") or x.get("text") or x.get("name") or "")
    ...
    # 2) 再从 why_you_examples 里兜底（你 app_demo.py 里已经在用这个函数）
    try:
        ex = get_why_you_examples(vertical)
        if isinstance(ex, list) and ex:
            # list[str]
            if all(isinstance(x, str) for x in ex):
                return ex
            # list[dict]
            out = []
            for x in ex:
                if isinstance(x, dict):
                    out.append(x.get("phrase") or x.get("text") or x.get("name") or x.get("bucket") or "")
            out = [s for s in out if s]
            if out:
                return out
    except Exception:
    ...
    # 3) 最后兜底（保证永不 crash）
    return ["省钱", "体验更好", "更省时间", "更简单", "更强爽感"]
def get_sell_point_options(vertical: str = "casual_game") -> list[str]:
    """
    给 variant_suggestions.py 用的 sell_point 列表。
    """
    try:
        v = get_pool(vertical, "sell_point")
        if isinstance(v, list):
            # 允许 list[str] 或 list[dict]
            if all(isinstance(x, str) for x in v):
                return v
            out = []
            for x in v:
                if isinstance(x, dict):
                    out.append(x.get("text") or x.get("name") or x.get("sell_point") or "")
            out = [s for s in out if s]
            if out:
                return out
    except Exception:
